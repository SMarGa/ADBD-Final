...
BEGIN
    ...
    SELECT Tipo INTO tipo_vehiculo FROM Vehiculo WHERE Matricula = NEW.Matricula;

    IF tipo_carnet_transportista = 'B' AND tipo_vehiculo = 'transporte_residuos' THEN
        RAISE EXCEPTION 'El transportista con carnet tipo B no puede conducir vehículos de transporte de residuos.';
    END IF;
...
-- Pruebas: probar las eliminaciones con restricciones de integridad, es decir las FK, probar los trigger y checks, operaciones de actualizacion, consultas..
